...
| Minimal PII | L | **CONFLICTS** | **4,728 contractor mobile numbers committed** in the BBMP CSV; `redactContacts` runs only at render time (one caller). Also `photos[].exif.{lat,lng,make,model}` public via `toPublicCase` | **EXTEND** — P0, `docs/PUBLIC_RELEASE.md` |
