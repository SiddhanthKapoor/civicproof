...
export function deriveNextActions(
  ...
  if (authority?.grievance) {
    actions.push({
      ...
      rationale: `Submit the complaint packet through ${authority.grievance.name}${authority.grievance.note ? ` (${authority.grievance.note})` : ""}, then record the reference number here so the case can be followed up.`,
